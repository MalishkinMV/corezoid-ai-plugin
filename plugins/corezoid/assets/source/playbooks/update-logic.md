...
2. **Create new variable if needed**: If the variable doesn't exist, create it using:
   ```bash
   API_URL=<API_URL> API_TOKEN=<API_TOKEN> WORKSPACE_ID=<WORKSPACE_ID> PROJECT_ID=<PROJECT_ID> ./convctl.sh create-variable <ROOT_FOLDER_ID> NAME DESCRIPTION VALUE
   ```
...
2. **Create the variable**:
   ```bash
   API_URL=https://admin.corezoid.com API_TOKEN=123 WORKSPACE_ID=123 PROJECT_ID=123 ./convctl.sh create-variable 123 api-endpoint "Main API Endpoint" "https://api.example.com"
   ```
